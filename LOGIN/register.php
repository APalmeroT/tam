	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			...
			if ($result) {
				...
				$_POST['password'] = "";
				$_POST['cpassword'] = "";
				echo "<script> window.location='/tamaolipas/LOGIN/index.php'</script>";
			} else {
				echo "<script>alert('Woops! Algo no salio como debia.')</script>";
			}
		} else {
			echo "<script>alert('Woops! Este correo ya existe.')</script>";
		}
...
<body>
	<div class="container">
		<form action="" method="POST" class="login-email">
            <p class="login-text" style="font-size: 2rem; font-weight: 800;">Crear cuenta</p>
			...
			<div class="input-group">
				<button name="submit" class="btn">Crear</button>
			</div>
			<p class="login-register-text">¿Tiene una cuenta? <a href="index.php">Iniciar sesion</a>.</p>
		</form>
	</div>
</body>
</html>